 ...
 * Created by pavankovurru on 1/29/17.
 ...
 //NOTE --> EXTRA DESIRED CAPABILITIES --> INTRODUCE IN THE PROJECT AS PER REQUIREMENTS


 1)   Appium comes with a default capability to accept, dismiss alerts
      cap.SetCapability("autoAcceptAlerts", true);
      cap.SetCapability("autoDismissAlerts", true);

 2)   Appium  will auto-switch to web-view in case of native apps
      cap.SetCapability("autoWebview", true);

 3)   UI AUTOMATOR 2
      cap.setCapability(MobileCapabilityType.AUTOMATION_NAME, AutomationName.ANDROID_UIAUTOMATOR2);

 4)  capabilities.setCapability("xcodeOrgId", "7Y5J2RJXYV");
     capabilities.setCapability("xcodeSigningId", "iPhone Developer");

 to run apps on real IOS DEVICE
 */
...
public class AppiumUtil {
    ...
    public static AppiumDriver createLocalIOSDriver_For_NativeApp_In_IOSDEVICE(String appName,String deviceName,String orgID){
    ...
    public static AppiumDriver createLocalIOSDriver_For_WebApp_In_IOSDEVICE(String browserName,String deviceName,String orgID){
    ...
    public static void horizontalSwipe(String mainContainerElement , String classNameOFSubElements){
        ...
        AndroidElement gallery = (AndroidElement) driver.findElementById(mainContainerElement);
        List<MobileElement> images = gallery
                .findElementsByClassName(classNameOFSubElements);
    ...
    public static void hover(WebDriver driver, By locator ) {
        ...
        action.moveToElement(AppiumUtil.wait_until_ElementIs_Visible(driver,locator)).build().perform();
    ...
    public static void hoverAndClick(WebDriver driver, By locator ) {
        Actions action = new Actions(driver);
        action.moveToElement(AppiumUtil.wait_until_ElementIs_Visible(driver,locator)).click().build().perform();
    ...
    public static void hoverClickAndSendData(WebDriver driver, By locator , String data ) {
        Actions action = new Actions(driver);
        action.moveToElement(AppiumUtil.wait_until_ElementIs_Visible(driver,locator)).click().sendKeys(data).build().perform();
        AppiumUtil.waitForPageToLoad(driver);
    }
    ...
    public static void hoverClickSendDataAndPressEnter(WebDriver driver, By locator , String data ) {
        Actions action = new Actions(driver);
        action.moveToElement(AppiumUtil.wait_until_ElementIs_Visible(driver,locator)).click().sendKeys(data).sendKeys(Keys.ENTER).build().perform();
        AppiumUtil.waitForPageToLoad(driver);
    }
    ...
    public static void hoverAndSendData(WebDriver driver, By locator , String data ) {
        Actions action = new Actions(driver);
        action.moveToElement(AppiumUtil.wait_until_ElementIs_Visible(driver,locator)).sendKeys(data).build().perform();
        AppiumUtil.waitForPageToLoad(driver);
    }
    ...
    public static void doubleClick(WebDriver driver, By locator) {
        Actions doubleClick = new Actions(driver);
        doubleClick.doubleClick(AppiumUtil.wait_until_ElementIs_Visible(driver,locator)).build().perform();
        AppiumUtil.waitForPageToLoad(driver);
    }
    ...
    public static void switchToNewWindowHandle_After_ClickingOnGivenElement (
        ...
        if (driver.getWindowHandles().size()>= 2) { //switch to a new window handle if there more than 1 window handles.
